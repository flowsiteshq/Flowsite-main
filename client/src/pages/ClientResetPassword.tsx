...
export default function ClientResetPassword() {
  ...
  const resetMutation = trpc.clientPortalAuth.resetPassword.useMutation({
    onSuccess: () => {
      setDone(true);
      ...
      <div className="min-h-screen bg-[oklch(0.065_0.005_260)] flex items-center justify-center p-4">
        <Card className="bg-white/5 border border-white/10 backdrop-blur-xl max-w-md w-full">
          <CardContent className="pt-8 pb-8 text-center">
            ...
            <p className="text-white/50 text-sm">This reset link is missing or invalid. Please request a new one from the login page.</p>
          </CardContent>
        </Card>
      </div>
    );
          ...
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-white/50 text-center">
              {done
                ...
                : "Enter a new password for your client portal account."}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {done ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Button
                  type="submit"
                  disabled={resetMutation.isPending || !isValid || !matches}
