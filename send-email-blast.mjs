/**
 * Email blast script — sends the FlowSites promotional email to all wizard_submissions leads
 * plus vincent.holmes00@gmail.com
 */
...
<body>
  <div class="wrapper">
    <!-- Header -->
    <div class="header">
      <div class="header-logo">Flow<span>Sites</span></div>
    </div>
    ...
    <div class="body">
      ...
      <div class="benefit">
        ...
        <div class="benefit-text">
          ...
          <span>Every page is engineered to turn visitors into customers, with strategic CTAs and proven layouts.</span>
        </div>
      </div>
      <div class="benefit">
        <div class="benefit-icon">✅</div>
        <div class="benefit-text">
          ...
          <span>Your site connects directly to your CRM so every lead is captured and followed up automatically.</span>
        </div>
      </div>
      <div class="benefit">
        <div class="benefit-icon">✅</div>
        <div class="benefit-text">
          <strong>Fast, mobile-first, SEO-ready</strong>
          <span>Your site loads fast, looks great on phones, and gets found on Google.</span>
        </div>
      </div>
      <div class="benefit">
        <div class="benefit-icon">✅</div>
        <div class="benefit-text">
          ...
          <span>We handle everything. Design, development, integration, and launch.</span>
        </div>
      </div>
      ...
      <div class="cta-section">
        <a href="https://flow-sites.com/interested">Yes, I'm Interested →</a>
      </div>
...
async function main() {
  // Connect to DB and get all leads
  const conn = await mysql.createConnection(DATABASE_URL);
  const [rows] = await conn.execute(
    "SELECT id, businessName, email FROM wizard_submissions WHERE email IS NOT NULL AND email != '' ORDER BY id ASC"
  );
  await conn.end();
  ...
  const allRecipients = [
    ...rows,
    { id: 0, businessName: "Vincent", email: "vincent.holmes00@gmail.com" },
  ];

  console.log(`Sending to ${allRecipients.length} recipients...`);

  let sent = 0;
  let failed = 0;
  const errors = [];

  for (const lead of allRecipients) {
    try {
      const result = await resend.emails.send({
        from: "FlowSites <noreply@flow-sites.com>",
        to: lead.email,
        subject: "Your Business Deserves a Website That Actually Works — Starting at $499",
        html: buildEmailHtml(lead.businessName || "there"),
      });
      ...
      if (result.error) {
        ...
        sent++;
        console.log(`✅ Sent: ${lead.email} (${sent}/${allRecipients.length})`);
      }
